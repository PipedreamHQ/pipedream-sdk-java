/**
 * This file was auto-generated by Fern from our API Definition.
 */
package com.pipedream.api.types;

import com.fasterxml.jackson.annotation.JsonAnyGetter;
import com.fasterxml.jackson.annotation.JsonAnySetter;
import com.fasterxml.jackson.annotation.JsonIgnore;
import com.fasterxml.jackson.annotation.JsonIgnoreProperties;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonSetter;
import com.fasterxml.jackson.annotation.Nulls;
import com.fasterxml.jackson.databind.annotation.JsonDeserialize;
import com.pipedream.api.core.Nullable;
import com.pipedream.api.core.NullableNonemptyFilter;
import com.pipedream.api.core.ObjectMappers;
import java.util.HashMap;
import java.util.Map;
import java.util.Objects;
import java.util.Optional;
import org.jetbrains.annotations.NotNull;

@JsonInclude(JsonInclude.Include.NON_ABSENT)
@JsonDeserialize(builder = Project.Builder.class)
public final class Project {
    private final String id;

    private final String name;

    private final Optional<String> appName;

    private final Optional<String> supportEmail;

    private final Optional<String> externalUrl;

    private final Map<String, Object> additionalProperties;

    private Project(
            String id,
            String name,
            Optional<String> appName,
            Optional<String> supportEmail,
            Optional<String> externalUrl,
            Map<String, Object> additionalProperties) {
        this.id = id;
        this.name = name;
        this.appName = appName;
        this.supportEmail = supportEmail;
        this.externalUrl = externalUrl;
        this.additionalProperties = additionalProperties;
    }

    /**
     * @return Hash ID for the project
     */
    @JsonProperty("id")
    public String getId() {
        return id;
    }

    /**
     * @return Display name of the project
     */
    @JsonProperty("name")
    public String getName() {
        return name;
    }

    /**
     * @return App name shown to Connect users
     */
    @JsonIgnore
    public Optional<String> getAppName() {
        if (appName == null) {
            return Optional.empty();
        }
        return appName;
    }

    /**
     * @return Support email configured for the project
     */
    @JsonIgnore
    public Optional<String> getSupportEmail() {
        if (supportEmail == null) {
            return Optional.empty();
        }
        return supportEmail;
    }

    /**
     * @return External URL for the project, if configured
     */
    @JsonIgnore
    public Optional<String> getExternalUrl() {
        if (externalUrl == null) {
            return Optional.empty();
        }
        return externalUrl;
    }

    @JsonInclude(value = JsonInclude.Include.CUSTOM, valueFilter = NullableNonemptyFilter.class)
    @JsonProperty("app_name")
    private Optional<String> _getAppName() {
        return appName;
    }

    @JsonInclude(value = JsonInclude.Include.CUSTOM, valueFilter = NullableNonemptyFilter.class)
    @JsonProperty("support_email")
    private Optional<String> _getSupportEmail() {
        return supportEmail;
    }

    @JsonInclude(value = JsonInclude.Include.CUSTOM, valueFilter = NullableNonemptyFilter.class)
    @JsonProperty("external_url")
    private Optional<String> _getExternalUrl() {
        return externalUrl;
    }

    @java.lang.Override
    public boolean equals(Object other) {
        if (this == other) return true;
        return other instanceof Project && equalTo((Project) other);
    }

    @JsonAnyGetter
    public Map<String, Object> getAdditionalProperties() {
        return this.additionalProperties;
    }

    private boolean equalTo(Project other) {
        return id.equals(other.id)
                && name.equals(other.name)
                && appName.equals(other.appName)
                && supportEmail.equals(other.supportEmail)
                && externalUrl.equals(other.externalUrl);
    }

    @java.lang.Override
    public int hashCode() {
        return Objects.hash(this.id, this.name, this.appName, this.supportEmail, this.externalUrl);
    }

    @java.lang.Override
    public String toString() {
        return ObjectMappers.stringify(this);
    }

    public static IdStage builder() {
        return new Builder();
    }

    public interface IdStage {
        /**
         * <p>Hash ID for the project</p>
         */
        NameStage id(@NotNull String id);

        Builder from(Project other);
    }

    public interface NameStage {
        /**
         * <p>Display name of the project</p>
         */
        _FinalStage name(@NotNull String name);
    }

    public interface _FinalStage {
        Project build();

        /**
         * <p>App name shown to Connect users</p>
         */
        _FinalStage appName(Optional<String> appName);

        _FinalStage appName(String appName);

        _FinalStage appName(Nullable<String> appName);

        /**
         * <p>Support email configured for the project</p>
         */
        _FinalStage supportEmail(Optional<String> supportEmail);

        _FinalStage supportEmail(String supportEmail);

        _FinalStage supportEmail(Nullable<String> supportEmail);

        /**
         * <p>External URL for the project, if configured</p>
         */
        _FinalStage externalUrl(Optional<String> externalUrl);

        _FinalStage externalUrl(String externalUrl);

        _FinalStage externalUrl(Nullable<String> externalUrl);
    }

    @JsonIgnoreProperties(ignoreUnknown = true)
    public static final class Builder implements IdStage, NameStage, _FinalStage {
        private String id;

        private String name;

        private Optional<String> externalUrl = Optional.empty();

        private Optional<String> supportEmail = Optional.empty();

        private Optional<String> appName = Optional.empty();

        @JsonAnySetter
        private Map<String, Object> additionalProperties = new HashMap<>();

        private Builder() {}

        @java.lang.Override
        public Builder from(Project other) {
            id(other.getId());
            name(other.getName());
            appName(other.getAppName());
            supportEmail(other.getSupportEmail());
            externalUrl(other.getExternalUrl());
            return this;
        }

        /**
         * <p>Hash ID for the project</p>
         * <p>Hash ID for the project</p>
         * @return Reference to {@code this} so that method calls can be chained together.
         */
        @java.lang.Override
        @JsonSetter("id")
        public NameStage id(@NotNull String id) {
            this.id = Objects.requireNonNull(id, "id must not be null");
            return this;
        }

        /**
         * <p>Display name of the project</p>
         * <p>Display name of the project</p>
         * @return Reference to {@code this} so that method calls can be chained together.
         */
        @java.lang.Override
        @JsonSetter("name")
        public _FinalStage name(@NotNull String name) {
            this.name = Objects.requireNonNull(name, "name must not be null");
            return this;
        }

        /**
         * <p>External URL for the project, if configured</p>
         * @return Reference to {@code this} so that method calls can be chained together.
         */
        @java.lang.Override
        public _FinalStage externalUrl(Nullable<String> externalUrl) {
            if (externalUrl.isNull()) {
                this.externalUrl = null;
            } else if (externalUrl.isEmpty()) {
                this.externalUrl = Optional.empty();
            } else {
                this.externalUrl = Optional.of(externalUrl.get());
            }
            return this;
        }

        /**
         * <p>External URL for the project, if configured</p>
         * @return Reference to {@code this} so that method calls can be chained together.
         */
        @java.lang.Override
        public _FinalStage externalUrl(String externalUrl) {
            this.externalUrl = Optional.ofNullable(externalUrl);
            return this;
        }

        /**
         * <p>External URL for the project, if configured</p>
         */
        @java.lang.Override
        @JsonSetter(value = "external_url", nulls = Nulls.SKIP)
        public _FinalStage externalUrl(Optional<String> externalUrl) {
            this.externalUrl = externalUrl;
            return this;
        }

        /**
         * <p>Support email configured for the project</p>
         * @return Reference to {@code this} so that method calls can be chained together.
         */
        @java.lang.Override
        public _FinalStage supportEmail(Nullable<String> supportEmail) {
            if (supportEmail.isNull()) {
                this.supportEmail = null;
            } else if (supportEmail.isEmpty()) {
                this.supportEmail = Optional.empty();
            } else {
                this.supportEmail = Optional.of(supportEmail.get());
            }
            return this;
        }

        /**
         * <p>Support email configured for the project</p>
         * @return Reference to {@code this} so that method calls can be chained together.
         */
        @java.lang.Override
        public _FinalStage supportEmail(String supportEmail) {
            this.supportEmail = Optional.ofNullable(supportEmail);
            return this;
        }

        /**
         * <p>Support email configured for the project</p>
         */
        @java.lang.Override
        @JsonSetter(value = "support_email", nulls = Nulls.SKIP)
        public _FinalStage supportEmail(Optional<String> supportEmail) {
            this.supportEmail = supportEmail;
            return this;
        }

        /**
         * <p>App name shown to Connect users</p>
         * @return Reference to {@code this} so that method calls can be chained together.
         */
        @java.lang.Override
        public _FinalStage appName(Nullable<String> appName) {
            if (appName.isNull()) {
                this.appName = null;
            } else if (appName.isEmpty()) {
                this.appName = Optional.empty();
            } else {
                this.appName = Optional.of(appName.get());
            }
            return this;
        }

        /**
         * <p>App name shown to Connect users</p>
         * @return Reference to {@code this} so that method calls can be chained together.
         */
        @java.lang.Override
        public _FinalStage appName(String appName) {
            this.appName = Optional.ofNullable(appName);
            return this;
        }

        /**
         * <p>App name shown to Connect users</p>
         */
        @java.lang.Override
        @JsonSetter(value = "app_name", nulls = Nulls.SKIP)
        public _FinalStage appName(Optional<String> appName) {
            this.appName = appName;
            return this;
        }

        @java.lang.Override
        public Project build() {
            return new Project(id, name, appName, supportEmail, externalUrl, additionalProperties);
        }
    }
}
